...
export default function DashboardSection() {
  ...
  if (loading) return <div className="flex justify-center items-center h-screen text-white">Checking role...</div>;
      ...
      <div className="w-full max-w-[80%] px-4">
        ...
        <div className="bg-gradient-to-r from-yellow-700 to-yellow-600 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-lg">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-200 mt-1">Take the challenge to improve your overall performance.</p>
            <button className="mt-3 px-4 py-2 bg-yellow-500 hover:bg-yellow-600 rounded-lg text-black font-medium transition">
              Take Challenge →
            </button>
          </div>
          <div className="flex-shrink-0">
            <img src="/images/running.png" alt="challenge" className="w-40 mx-auto md:mx-0" />
          </div>
        </div>
        ...
        <div className="flex gap-6 mt-4 border-b border-yellow-600 overflow-x-auto">
          <button
            className={`pb-2 whitespace-nowrap ${activeTab === 'level1' ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-gray-400'}`}
            onClick={() => setActiveTab('level1')}
          >
            Level 1 Course
          </button>
          <button
            className={`pb-2 whitespace-nowrap ${activeTab === 'conceptual' ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-gray-400'}`}
        ...
        <div className="mt-6 flex flex-col gap-6">
          {loadingCourses ? (
            ...
            courses.map((course) => (
              <div key={course.id} className="bg-[#1b1b2f] rounded-xl flex flex-col md:flex-row overflow-hidden shadow-lg">
                <img src={course.banner || '/images/course1.png'} alt={course.title} className="w-full md:w-60 object-cover flex-shrink-0" />
                <div className="flex-1 p-6 flex flex-col justify-between">
                  <div>
                    <h3 className="text-lg font-semibold mb-1">{course.title}</h3>
                    <p className="text-sm text-gray-400 mb-4">Instructor Name</p>
                  ...
                  <div className="flex flex-wrap gap-3 mt-4">
                    <button
                      onClick={() => router.push(`/dashboard/courses/${course.documentId}`)}
